...
use App::cnext::cmd::cpanfile    ();
use App::cnext::cmd::fromtarball ();
use App::cnext::cmd::getrepo     ();
use App::cnext::cmd::help        ();
use App::cnext::cmd::install     ();
use App::cnext::cmd::look        ();
use App::cnext::cmd::selfinstall ();
use App::cnext::cmd::selfupdate  ();
use App::cnext::cmd::search      ();
use App::cnext::cmd::start       ();
use App::cnext::cmd::test        ();
use App::cnext::cmd::version     ();

use App::cnext::Index::Repositories;
use App::cnext::Index::Modules;
...
sub _build_http {
    App::cnext::Http->create;    # FIXME maybe some args
}

sub _build_repositories_idx($self) {
    App::cnext::Index::Repositories->new( cli => $self );
}

sub _build_modules_idx($self) {
    App::cnext::Index::Modules->new( cli => $self );
}
...
sub parse_options ( $self, @opts ) {
    ...
    $App::cnext::Logger::COLOR         = 1 if $self->{color};
    $App::cnext::Logger::VERBOSE       = 1 if $self->{verbose};
    $App::cnext::Logger::DEBUG         = 1 if $self->{debug};
    $App::cnext::Logger::SHOW_PROGRESS = 1 if $self->{show_progress};
...
sub get_cmd_sub_for ( $self, $cmd ) {
    ...
    return "App::cnext::cmd::$cmd"->can('run');
}
