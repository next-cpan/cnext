...
sub handle_app_exit {
    ...
    my $exclusive = $self->get_app_cumulative_time - $self->get_run_cumulative_time;
    $self->_emit(
    ...
    my $overhead = $self->get_run_cumulative_time - $self->get_sys_cumulative_time;
    $self->_emit(
...
sub handle_run_exit {
    ...
    $self->_emit(
        "IPC::Run3 run3()  : ", _t( $self->get_run_time ), "\n",
